...
class BiginnerActivity : AppCompatActivity(), SensorEventListener {
    ...
    internal lateinit var delay: Runnable
    internal lateinit var delayStartCountDown: Runnable
    private var enablestart: Runnable? = null
    // 3分= 3x60x1000 = 180000 msec
    internal var countNumber: Long = 20000
    //スタート前
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val countDown_before = CountDown(10000, interval)
        ...
        startButton.setOnClickListener {
            ...
            val dialogFragment_setpoketto = AlertDialogFragment_setpoketto()
            // DialogFragmentの表示
            ...
            val xx = 0
            while (set_frag > 1) {
                handler.postDelayed(enablestart!!, ((set_frag - 1) * 30000 + 100).toLong())
                handler.postDelayed(
                    delayStartCountDown,
                    ((set_frag - 1) * 30000 + 7000).toLong()
                )//遅延実行
                handler.postDelayed(
                    delayStartCountDown,
                    ((set_frag - 1) * 30000 + 8000).toLong()
                )//遅延実行
                handler.postDelayed(
                    delayStartCountDown,
                    ((set_frag - 1) * 30000 + 9000).toLong()
                )//遅延実行
                handler.postDelayed(delay, ((set_frag - 1) * 30000 + 10001).toLong())//遅延実行
                set_frag--
            }
            set_frag = set_frag_c
            setCount!!.text = "×" + set_frag + "セット"
    ...
    class AlertDialogFragment_setpoketto : DialogFragment() {
        internal val handler = Handler()
        private var delayClose: Runnable? = null
...
private fun Button.setOnClickListener(function: (View) -> Unit) {
