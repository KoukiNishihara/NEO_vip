...
class AdvancedActivity : AppCompatActivity(), SensorEventListener {
    ...
    private var timerText: TextView? = null//タイマーの表示文
    ...
    internal var countNumber: Long = 40000
    //スタート前
    ...
    lateinit var setCountButton: Button
    //private Runnable;
    ...
    public override fun onCreate(savedInstanceState: Bundle?) {
        ...
        startButton.setOnClickListener {
            ...
            val xx = 0
            while (set_frag > 1) {
                handler.postDelayed(enablestart!!, ((set_frag - 1) * 50000 + 100).toLong())
                handler.postDelayed(
                    delayStartCountDown!!,
                    ((set_frag - 1) * 50000 + 7000).toLong()
                )//遅延実行
                handler.postDelayed(
                    delayStartCountDown!!,
                    ((set_frag - 1) * 50000 + 8000).toLong()
                )//遅延実行
                handler.postDelayed(
                    delayStartCountDown!!,
                    ((set_frag - 1) * 50000 + 9000).toLong()
                )//遅延実行
                handler.postDelayed(delay!!, ((set_frag - 1) * 50000 + 10001).toLong())//遅延実行
                set_frag--
            }
            set_frag = set_frag_c
            setCount!!.text = "×" + set_frag + "セット"
    ...
    class AlertDialogFragment_checkScore : DialogFragment() {

        private val No1: Int = 0
        private val No2: Int = 0
        private val No3: Int = 0
